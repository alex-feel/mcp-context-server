...
Tables: `context_entries` (main; thread_id/source indexes, JSON metadata, summary column), `tags` (many-to-many, lowercase, UNIQUE on `(context_entry_id, tag)` — tags are a SET per entry; the write path normalizes and deduplicates through `app/models.py::normalize_tag_list`, the single definition of what makes two tags the same label, and the inserts carry `ON CONFLICT DO NOTHING` on that index), `image_attachments` (binary, cascade delete), `context_index_nodes` (index_tree per-node summaries; FK to `context_entries(id)` ON DELETE CASCADE — TEXT on SQLite, UUID on PostgreSQL; created only when `ENABLE_INDEX_TREE_NODE_SUMMARIES` is on).
