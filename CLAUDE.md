...
**FastMCP Logging** (NOT in `app/settings.py`): `FASTMCP_ENABLE_RICH_LOGGING` (true*; set `false` in Docker/cloud), `FASTMCP_LOG_LEVEL`/`FASTMCP_LOG_ENABLED` (import-time; `config_logger` re-aligns the non-propagating `fastmcp` logger tree with `LOG_LEVEL` at startup, so `LOG_LEVEL` is the single effective verbosity control) — see [FASTMCP_* Governance](#fastmcp_-env-var-governance).
