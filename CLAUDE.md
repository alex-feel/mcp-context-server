...
**index_tree per-node summaries (ON by default, `ENABLE_INDEX_TREE_NODE_SUMMARIES`)**: the optional LLM layer stored in `context_index_nodes`. Generated in a SEPARATE fenced never-raise pass (`generate_index_nodes_with_timeout` in `_shared.py`) that reuses the summary provider via `SummaryProvider.summarize_with_prompt()` with a dedicated short prompt; a node-summary failure/timeout NEVER aborts a store. Written atomically inside the store/update transaction via `IndexNodeRepository.replace_nodes_for_context` (which pre-checks table existence so a missing table is a no-op, never an abort).

### Migration System

Auto-applied idempotent migrations in `app/migrations/`: semantic search, FTS, chunking (1:N embeddings), metadata indexing, summary column, index_tree (`context_index_nodes`, gated on `ENABLE_INDEX_TREE_NODE_SUMMARIES`). PostgreSQL DDL uses `POSTGRESQL_MIGRATION_TIMEOUT_S` (300s). Changing `FTS_LANGUAGE` requires an FTS table rebuild.
