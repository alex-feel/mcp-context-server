...
2. **Authentication Layer** (`app/auth/`): HTTP-transport auth via `MCP_AUTH_PROVIDER`; architecturally inert on stdio (`create_auth_provider()` is only called for non-stdio transports, so misconfigured auth never blocks a stdio start). `simple_token.py`: static bearer token, constant-time comparison (`MCP_AUTH_TOKEN`). The `jwt` provider (factory arm in `__init__.py`) verifies IdP-issued JWTs through fastmcp's `JWTVerifier` — static PEM key/HS secret OR JWKS URI (exactly one; validated in the factory, NOT in pydantic, to preserve stdio inertness), optional issuer/audience pinning, `MCP_AUTH_JWT_*` env vars. `claims.py`: IdP-agnostic claim extraction (flat, dotted-path, and full-URL claim keys; Entra ID group-overage fail-closed to empty groups, no Graph call). `principal.py`: `RequestPrincipal(principal_id, groups, roles)` resolved per-request from `fastmcp.server.dependencies.get_access_token()` (`sub` claim, client-id fallback); returns `None` without a verified token. `access.py`: access-control policy — `resolve_effective_principal()` maps the no-token case to `ACCESS_CONTROL_DEFAULT_PRINCIPAL` (empty groups/roles) so every request resolves to exactly one owner identity, and `visibility_denied_reason()` enforces the `ACCESS_CONTROL_PUBLISH_ROLE` publish gate. NOTE: the jwt provider authenticates and writes are owner-stamped, but reads are NOT yet visibility-scoped (read-path enforcement is not implemented); docs mark multi-principal isolation experimental.
...
3. **Storage Backend Layer** (`app/backends/`):
   ...
   - **PostgreSQLBackend**: async via asyncpg, connection pooling, MVCC, JSONB/GIN indexes, pgvector, Pgpool-II auto-detection (warns to set `POSTGRESQL_STATEMENT_CACHE_SIZE=0`). Sends NO startup-packet parameters: `search_path`, `extra_float_digits` and the TCP keepalive GUCs are applied as `SET` statements (`session_guc_set_statements` / `apply_session_gucs` in `postgresql_backend.py`) by the pool setup callback on every acquire — which also restores them after the pool's `RESET ALL` — and by each one-off connection right after dialing, so an external pooler cannot refuse a connection over a non-allowlisted startup parameter.
   - **Backend Factory** (`factory.py`): creates backend from `STORAGE_BACKEND`.

4. **Repository Pattern** (`app/repositories/`): **RepositoryContainer** (`__init__.py`) = DI container. Repositories: Context (CRUD, search, deduplication), Tag (normalization, many-to-many), Image (binary attachments), Statistics, Embedding (vector storage/search), Fts (FTS5/tsvector), Grant (per-entry access grants, idempotent inserts). All use the `StorageBackend` protocol (database-agnostic); `BaseRepository` provides `_placeholder()`, `_placeholders()`, `_json_extract()`.
...
Tables: `context_entries` (main; thread_id/source indexes, JSON metadata, summary column), `tags` (many-to-many, lowercase, UNIQUE on `(context_entry_id, tag)` — tags are a SET per entry; the write path normalizes and deduplicates through `app/models.py::normalize_tag_list`, the single definition of what makes two tags the same label, and the inserts carry `ON CONFLICT DO NOTHING` on that index), `image_attachments` (binary, cascade delete), `context_entry_grants` (per-entry access grants: `principal_type` user|group, `principal_id`, `permission` read|write, `granted_by`, UNIQUE on `(context_entry_id, principal_type, principal_id, permission)` with `ON CONFLICT DO NOTHING` idempotent inserts, FK ON DELETE CASCADE — TEXT FK on SQLite, UUID on PostgreSQL), `context_index_nodes` (index_tree per-node summaries; FK to `context_entries(id)` ON DELETE CASCADE — TEXT on SQLite, UUID on PostgreSQL; created only when `ENABLE_INDEX_TREE_NODE_SUMMARIES` is on).

**Access-control columns (`owner_id`, `visibility`)**: `context_entries.owner_id` (TEXT NOT NULL, no default in the base schema) is the server-stamped effective principal — the verified request principal, or `ACCESS_CONTROL_DEFAULT_PRINCIPAL` when no verified token exists (`app/auth/access.py::resolve_effective_principal`) — and is NEVER a tool parameter; ownership is immutable. `visibility` (TEXT NOT NULL DEFAULT 'private', CHECK IN ('private','shared','public')) is settable via the optional `visibility` parameter on the four write tools: owner-only on updates, gated by `ACCESS_CONTROL_PUBLISH_ROLE` for 'public' (`visibility_denied_reason`, applied to the EFFECTIVE value, so a caller-omitted visibility defaulting to 'public' is still gated), and applied on INSERT only for stores — a dedup UPDATE never changes owner or visibility, and the dedup COALESCE leg deliberately excludes both columns. `ACCESS_CONTROL_DEFAULT_GROUP_GRANTS=author_groups` additionally writes a read grant per author group into `context_entry_grants` inside the same store transaction (`GrantRepository.store_group_read_grants`, fresh INSERTs only). Lookup indexes: `(owner_id)`, `(owner_id, thread_id)`, partial WHERE `visibility='public'`, grants `(principal_type, principal_id, context_entry_id)`. Both columns are internal for now (absent from `CONTEXT_ENTRY_COLUMNS`); read-path visibility filtering is not enforced yet, so multi-principal isolation under the `jwt` provider remains experimental.
...
**Ranked pagination**: the three ranked tools (semantic, FTS, hybrid) decide their order after the database answers (reranking, RRF, or both), so every page is a window into ONE ordering computed over a FIXED candidate depth (`RANKED_SEARCH_DEPTH` = `MAX_SEARCH_LIMIT` = 100 in `app/tools/search.py`), independent of the requested `limit`/`offset`. A window reaching past the depth is served short and the response says so via `rank_depth_limit`; an offset at or past the depth is answered from the arguments alone, running no search and no embedding call (a structurally invalid filter still takes precedence and falls through to the normal reporting path). `fts_search_context` and `hybrid_search_context` bound their `query` at `MAX_FTS_QUERY_LENGTH` (10,000 characters), since that text is handed to a search engine that builds an expression from it; the semantic-only tool is unbounded (its query goes to an embedding provider).
...
**Auth**: `MCP_AUTH_PROVIDER` (none*/simple_token/jwt), `MCP_AUTH_TOKEN`, `MCP_AUTH_CLIENT_ID` (mcp-client*); jwt provider: `MCP_AUTH_JWT_PUBLIC_KEY` XOR `MCP_AUTH_JWT_JWKS_URI` (exactly one), `MCP_AUTH_JWT_ISSUER`, `MCP_AUTH_JWT_AUDIENCE`, `MCP_AUTH_JWT_ALGORITHM` (RS256*), `MCP_AUTH_GROUPS_CLAIM` (groups*), `MCP_AUTH_ROLES_CLAIM` (roles*)
...
**Embedding**: `EMBEDDING_PROVIDER` (ollama*/openai/azure/huggingface/voyage), `EMBEDDING_MODEL` (qwen3-embedding:0.6b*), `EMBEDDING_DIM` (1024*), `EMBEDDING_TIMEOUT_S` (240*), `EMBEDDING_MAX_CONCURRENT` (3*), `EMBEDDING_QUERY_INSTRUCTION` (unset*; opt-in prefix prepended verbatim to QUERY embeddings only — semantic + hybrid search; document embeddings on the store/update path always stay bare)
...
Schema-aware FastMCP middleware fixing MCP clients (including Claude Code) intermittently serializing list/dict parameters as JSON strings. `Middleware` base class with `on_call_tool` override; `build_schema_map()` inspects each tool's JSON Schema at startup for `array`/`object` params (incl. `Optional` via `anyOf`/`$ref`) — only those are deserialization candidates, strings are never touched. Handles double-encoding. Registered in `app/server.py` lifespan (step 27) via `mcp.add_middleware()`, after all `register_tool()` calls.
...
- **Removal** (when upstream clients fix serialization): delete `app/middleware/` + its import/registration block (step 27) in `app/server.py` lifespan(), delete `tests/middleware/test_middleware_json_deserializer.py`, remove middleware integration tests from `tests/integration/_harness.py`, and remove this section.
