...
- Compression provisioning follows embedding storage: with `ENABLE_EMBEDDING_GENERATION=false` on a FRESH database -- no compression schema and no embedding infrastructure -- the server creates neither the compressed table nor a provenance row (nothing can ever write a compressed payload on that configuration), so a later `ENABLE_EMBEDDING_COMPRESSION=false` flip has nothing to wedge on. A database that already carries the compression schema -- data compressed while generation was on -- keeps being maintained and seed-validated so the decode path stays protected, and a database whose embedding tables were ever provisioned (an `embedding_metadata` table exists) still receives the compression schema, because the delete/update cleanup paths route through the compressed payload table whenever compression is enabled. The enable-direction guard above applies REGARDLESS of the generation toggle: a populated fp32 table with no provenance row refuses to start (exit 78) either way.
