...
- Compression provisioning follows embedding storage: with `ENABLE_EMBEDDING_GENERATION=false` and no existing compression schema, the server creates neither the compressed table nor a provenance row (nothing can ever write a compressed payload on that configuration), so a later `ENABLE_EMBEDDING_COMPRESSION=false` flip has nothing to wedge on. A database that already carries the schema -- data compressed while generation was on -- keeps being maintained and seed-validated so the decode path stays protected.
